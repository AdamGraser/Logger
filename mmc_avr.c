...
void power_on (void)
{
	PORTB |= 0x10 | 0x20;			/* Configure SCK/MOSI/CS as output */
	DDRB  |= 0x10 | 0x20 | 0x80;
...
void power_off (void)
{
	...
	DDRB  &= 0x4F;			/* Set SCK/MOSI/CS as hi-z */
	PORTB &= 0x4F;
}
...
)
{
	...
	for (tmr = wt * 10; tmr; tmr--)
	{
		if((d = xchg_spi(0xFF)) == 0xFF) break;
		dly_us(100);
	}
...
)
{
	...
	{
		if((token = xchg_spi(0xFF)) != 0xFF) break;
		dly_us(100);
	}
